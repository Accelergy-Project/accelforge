...
def construct_spacetime(dims: list) -> list[Tag]:
    ...
    for dim in dims:
        if dim["type"] == "Temporal":
            ...
            spacetime.append(
                Spatial(dim["spatial_dim"], dim["target"])
            )
...
class TestSimpleMulticastModel(unittest.TestCase):
    ...
    def test_0(self):
        ...
        assert info.fulfilled_map.map_ == isl.Map(
            isl.DEFAULT_CONTEXT,
            "{ spacetime[t, x, y] -> data[t + x + y] : "
            "0 <= t < 4 and 0 <= x < 2 and 0 <= y < 2 }"
        )
        assert info.parent_reads.map_ == isl.Map(
            isl.DEFAULT_CONTEXT,
        ...
        assert info.fulfilled_fill.map_ == isl.Map(
            isl.DEFAULT_CONTEXT,
            "{ spacetime[t, x, y] -> data[t + x + y] : "
            "0 <= t < 4 and 0 <= x < 2 and 0 <= y < 2 }"
        )
...
class TestHypercubeMulticastModel(unittest.TestCase):
    TEST_CASES_FILE: str = Path(__file__).parent / "multicast"
    testcases: dict = load_solutions(TEST_CASES_FILE / "multicast" / "test_cases.yaml")
    ...
    def test_gamut(self):
        for test in testcases:
            ...
            if test["expected"]["hypercube_hops"] is None:
                ...
                assert ret == test["expected"]["hypercube_hops"]
