...
class TestCommunicationLatency(unittest.TestCase):
    ...
    def test_fused_repays_communication_latency_each_switch(self):
        ...
        # via the Einsum delay. MatmulA: I's and A's GlobalBuffer connections
        # (write + read = 30 each) plus one MAC: (30 + 3 + 30) x 4 = 252.
        # MatmulB mirrors it with A's and B's GlobalBuffer connections:
        # (30 + 3 + 30) x 4 = 252. Paid once: the inputs' MainMemory reads
        # (descent, max = 100) and B's MainMemory write (ascent, 200; the first
...
class TestSharedMemoryOverlap(unittest.TestCase):
    def test_fused_memory_bound_overlaps_compute(self):
        ...
        so their reads may fill any slice of the fused execution, while I/B's
        ...
        # MatmulA is compute-bound: 64 MACs / 0.2 = 320 vs reading I and WA from
        # MainMemory (256 bits / 1 = 256). MatmulB is memory-bound: writing B back
        # (128 bits / 0.1 = 1280) plus reading WB (128 / 1 = 128) is 1408 vs 320.
        self.assertEqual(row["MatmulA<SEP>component_latency<SEP>MAC"], 320)
        self.assertEqual(row["MatmulA<SEP>component_latency<SEP>MainMemory"], 256)
        self.assertEqual(row["MatmulB<SEP>component_latency<SEP>MAC"], 320)
        self.assertEqual(row["MatmulB<SEP>component_latency<SEP>MainMemory"], 1408)

        # max(320 + 320, 256 + 1408) = 1664: MatmulA's MainMemory slack absorbs part
        # of MatmulB's traffic. Summing per-Einsum maxes would give 320 + 1408 = 1728.
    ...
    def test_separate_read_write_ports(self):
        ...
        components that may overlap: WB's reads no longer serialize behind B's
        ...
        self.assertEqual(
            row["MatmulA<SEP>component_latency<SEP>MainMemory (read)"], 256
        )
        self.assertEqual(
            row["MatmulB<SEP>component_latency<SEP>MainMemory (read)"], 128
        )
        self.assertEqual(
            row["MatmulB<SEP>component_latency<SEP>MainMemory (write)"], 1280
        )
        # B's writes go to its GlobalBuffer staging below the fused m loop, so
        # the write port's 1280 is confined to MatmulB's block and sets its
    ...
    def test_transfer_needs_deeper_reservation(self):
        ...
        credit above is lost. MatmulA is compute-bound (320 vs reading I once and
        WA per m iteration: (128 + 4 x 128) / 4 = 160); MatmulB's writeback still
...
class TestLatencyTimeline(unittest.TestCase):
    ...
    def test_timeline_overlap_layout(self):
        ...
        I/B traffic sits at level 1 (their GlobalBuffer reservations live below
        the fused m loop, confining it to its own Einsum's block); weight reads sit
        at level 0 and may fill slack anywhere. MatmulB's B writeback fills its
        ...
        self.assertEqual(
            ...
            [("MatmulA", 0, 320), ("MatmulB", 320, 1664)],
        )
        mm = [
        ...
        self.assertEqual(
            sorted(mm),
            [
                ("MatmulA", 0, 128, 256),  # WA reads, shareable
                ("MatmulA", 1, 0, 128),  # I reads, private
                # WB reads, shareable; right-aligned to the 1664 deadline
                ("MatmulB", 0, 1536, 1664),
                ("MatmulB", 1, 320, 1600),  # B writeback, private
            ],
        )
        ...
        self.assertEqual(mac, [("MatmulA", 0, 320), ("MatmulB", 320, 640)])
