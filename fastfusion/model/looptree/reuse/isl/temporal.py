...
from fastfusion.model.looptree.reuse.isl.mapping_to_isl.types import TEMPORAL_TAGS, Fill, Occupancy

@dataclass(frozen=True)
class TemporalReuseSettings:
    occupancy: Occupancy
    exploit_reuse: bool
    multiple_loop_reuse: bool
...
def fill_from_occupancy(
    ...
    for dim_idx, tag in reversed(list(enumerate(occupancy.tags))):
        ...
        # Calculates the 
            ...
            time_domain_to_past: isl.Map = isl.Map.lex_gt(
                spacetime_domain_to_time_domain.range().get_space()
            ).intersect_domain(time_domain).intersect_range(time_domain)
            ...
            spacetime_domain_space_preserver: isl.Map = spacetime_domain_to_space_domain.apply_range(
